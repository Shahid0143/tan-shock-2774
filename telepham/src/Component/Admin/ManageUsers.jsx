...
const ManageUsers = () => {
  const { isLoadingUserList, isErrorUserList, users, orders, carts } =
  ...
  orders.forEach((order, i) => {
    if (!totalArray.some((obj) => obj.useremail === order.useremail)) {
      const user = {
        ...
        orderQuantity: 1,
        totalOrderPrice: order.originalPrice - order.discountPrice,
      };
      totalArray.push(user);
    } else {
      totalArray.some((obj) => {
        obj.totalOrderPrice =
          obj.totalOrderPrice + (order.originalPrice - order.discountPrice);
  ...
  users.forEach((user) => {
    if (!totalArray.some((obj) => obj.username === user.name))
      totalArray.push(user);
  });
                ...
                {totalArray.map((user) => {
                  ...
                  return (
                    <Tr key={user.id}>
                      ...
                      <Td>{1}</Td>
                      <Td>
                        ₹{user?.totalOrderPrice ? user.totalOrderPrice : 0}
                      </Td>
                      <Td>₹{totalProfit}</Td>
                      <Td>
                        <IconButton
